...
@Slf4j
@Component
public class SyncAllLocationTreeRunner implements ApplicationRunner {
    ...
    private void startCheckSessionTimeout() {
        log.info("startSyncAllLocationTree");
        Task task = new Task();

        //任务名称，描述这个任务具体的作用
        task.setName("同步全量位置树");
        //任务是周期任务还是单次任务，0为周期，1为单次
        task.setTaskExecType(TaskExecTypeEnum.PERIOD.getCode());
        task.setTaskPeriod("{\"corn\":\"0/30 * * * * ?\"}");
        task.setUuid("SyncAllLocationTreeTask");
        //设置任务类型，对应任务中心二级菜单
        task.setTaskType(8);
        //设置任务子类型,方便对任务进行过滤查询
        task.setSubTaskType("同步全量位置树");
        //任务的Metadata，可以用来传递任务的上下文
        Map<String, Object> meta = new HashMap<>();
        task.setMetadata(JSON.toJSONString(meta));
        //注册任务回调的类
        task.setCallback(SyncAllLocationTreeTask.class.toString().split("class ")[1]);
