...
@Slf4j
@Component
public class PassExpirationRunner implements ApplicationRunner {
    ...
    private void startCheckSessionTimeout() {
        ...
        task.setTaskPeriod("{\"corn\":\"0 0 9,11,13,15,17,19,21 * * ?\"}");
        task.setUuid("PassExpirationTask");
        // 设置任务类型，对应任务中心二级菜单
        task.setTaskType(8);
        // 设置任务子类型,方便对任务进行过滤查询
        task.setSubTaskType("密码到期提醒");
        // 任务的Metadata，可以用来传递任务的上下文
        Map<String, Object> meta = new HashMap<>();
        task.setMetadata(JSON.toJSONString(meta));
        // 注册任务回调的类
        task.setCallback(PassExpirationTask.class.toString().split("class ")[1]);
